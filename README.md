...
Формула ключевой метрики приходит в контракте `monitoring_metric` от
`laim-baskets-adapter` в явном виде — так, как она определена в отчёте о
валидации, например `mean(prediction == target)` или
`f1(prediction, target, "macro")`. Её исполняет общий пакет `laim_monitoring`
(`formula.py`), один и тот же у адаптера, ассесора и km-dynamic.

Судья (LLM) не ставит «оценку метрики». Он воспроизводит разметку человека из
эталонной корзины: истинный класс, оценки по критериям, голоса — все входы
формулы, кроме ответа агента (`prediction`), который наблюдается в UMR.
Разметка судьи записывается в `scored_data` в те же колонки, что в корзине,
и km-dynamic считает по ним ту же формулу, что baseline (`assessment_plan.py`).

Готовый score судья ставит только там, где формулу применить нельзя, и это
помечается в `assessment_result.scoring_semantics = judge_final_score` с
причиной: формула использует `prediction`, а трейсы его не несут; либо
единица оценки — целый диалог. Иначе `scoring_semantics = contract_formula`.

Калибровка и мониторинг используют одно решение на запуск, поэтому `acc_auto`
(доля точных совпадений разметки судьи с человеком на holdout корзины)
измеряет ровно то, чем потом размечается мониторинг.

...
Контракт `monitoring_metric` от `laim-baskets-adapter` несёт формулу КМ так,
как она определена в отчёте о валидации (`mean(prediction == target)`,
`f1(prediction, target, "macro")`, `mean(оценка >= 4)`), и её входы: колонки
UMR с признаком `judged`. Формулу исполняет общий пакет `laim_monitoring`,
один и тот же у адаптера, ассесора и km-dynamic.

Судья (LLM) не ставит «оценку метрики». Он воспроизводит входы с
`judged=true` — то, что в эталонной корзине проставлял человек (истинный
класс, критерии, голоса). Ответ агента (`judged=false`) берётся из UMR.
...
ответ агента не наблюдается в трейсах либо единица — целый диалог. Тогда
`assessment_result.scoring_semantics = judge_final_score` с причиной, иначе
`contract_formula`. Если судья не ответил более чем на 20% единиц,
`assessment_result.status = not_computable`.

`acc_auto` — доля точных совпадений разметки судьи с человеком на holdout
эталонной корзины; калибровка и мониторинг используют одно решение о том, что
размечает судья.

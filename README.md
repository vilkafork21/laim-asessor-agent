...
Эталон делится на train/holdout со стратификацией по оценке: дефекты в корзинах
редки (5-12%), и при случайном делении holdout остаётся почти без них.
Оценки на нижней границе шкалы проходят второй проход (`REVIEW_PROMPT`): судья
ищет правило инструкции, по которому снижать не следует, — это убирает ложные
срабатывания на корректных по конвенциям ответах.
В few-shot гарантированы примеры нарушений (отдельный индекс по ним): в top-k
похожих их попадает не больше одного, и судья перестаёт узнавать нарушение.

Метрики считаются на calibration-holdout между судьёй и человеческой разметкой,
...
- **Объём holdout** (`holdout_units`, `holdout_defect_units`): на скольких единицах
  посчитаны метрики и сколько среди них с минимальной оценкой. Каппа и альфа при
  единицах минорного класса неустойчивы — судить по ним о судье нельзя
...
- **Полнота и точность на дефектах** (`defect_recall`, `defect_precision`): обычные
  recall и precision, где искомый класс — нарушение (оценка на нижней границе шкалы).
  `defect_recall` — какую долю размеченных нарушений судья нашёл, `defect_precision` —
  какая доля сниженных им оценок оказалась верной. При перекошенной разметке судья,
  зачитывающий всё подряд, показывает высокую accuracy — деградацию видно только здесь

Судья видит только то, что пришло в UMR. Нарушения, которые распознаются лишь по
знанию предметной области (недостоверные условия продукта, неверный маршрут), он не
выявляет и по инструкции не должен их угадывать. На таких корзинах `defect_recall`
близок к нулю: это означает, что класс нарушений судьёй не покрыт, а не что расчёт
сломан. Замер на CI09840650: recall 0.000 при любой конфигурации судьи.

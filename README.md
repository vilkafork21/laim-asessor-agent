...
строится на всём размеченном эталоне; допуска калибровки у этого запуска нет. Для `accuracy` контекст строится из `prediction`, как в эталоне.
Отсутствующая колонка или хотя бы одно пустое `prediction` останавливает
оценку до создания моделей: `not_computable/missing_prediction`. Все строки
сохраняются с `main_metric=NaN`: исключение неполных строк меняло бы
оцениваемую совокупность. Подмена задачи оценкой текста ответа запрещена. Обход трейсов — тот же, что в п. 6.
...
| `accuracy`: отсутствует колонка prediction или есть пустое значение | `not_computable`, `missing_prediction`; строки сохранены без оценок, модели не вызываются |
